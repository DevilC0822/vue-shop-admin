<template>
  <div class="users-view">
    ...
    <el-row :gutter="20">
      <el-col :span="12">
        <div class="grid-content bg-purple">
          <div style="margin-top: 15px">
            <el-input placeholder="请输入查找内容" v-model="getUsersParams.query" class="input-with-select" clearable @clear="getUserInfo" @keyup.enter.native="getUserInfo">
      ...
      <el-col :span="3">
        <div class="grid-content bg-purple">
          <el-button type="primary" icon="el-icon-circle-plus-outline" @click="addUserDialog = true">添加用户</el-button>
        </div>
      </el-col>
    </el-row>
    ...
    <el-table :data="usersInfo" border style="width: 100%">
      ...
      <el-table-column fixed="right" label="操作" align="center" max-width="250" min-width="150">
        <template slot-scope="scope">
          <el-button @click="editUser(scope.row.id)" type="text" size="small">修改</el-button>
    ...
    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="getUsersParams.pagenum" :page-sizes="[1, 2, 5, 10]" :page-size="getUsersParams.pagesize" layout="total, sizes, prev, pager, next, jumper" :total="usersTotal">
    </el-pagination>

    <!-- 添加用户弹出框 -->
    <el-dialog title="添加用户" :visible.sync="addUserDialog" :close-on-click-modal="false" @close="addUserFormClose">
      <el-form :model="addUserDialogForm" :rules="addUserRules" ref="addUserForm">
        <el-form-item label="用户名" label-width="70px" prop="username">
          <el-input v-model="addUserDialogForm.username" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="密码" label-width="70px" prop="password">
          <el-input v-model="addUserDialogForm.password" autocomplete="off" type="password" show-password></el-input>
        </el-form-item>
        <el-form-item label="手机号" label-width="70px" prop="mobile">
          <el-input v-model="addUserDialogForm.mobile" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="邮箱" label-width="70px" prop="email">
          <el-input v-model="addUserDialogForm.email" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <!-- 修改用户弹出框 -->
    <el-dialog title="修改用户" :visible.sync="editUserFormDialog" @close="editUserFormDialogClose">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editUserDialogSubmit">确 定</el-button>
      </div>
    </el-dialog>

    <!-- 分配角色弹出框 -->
    <el-dialog title="分配角色" :visible.sync="changeRoleDialog" :center="false">
      <p>当前的用户:{{currentUser.username}}</p>
      <p>当前的角色:{{currentUser.role_name}}</p>

      <span>分配新角色:</span>
      <el-select v-model="value" placeholder="请选择">
        <el-option v-for="item in rightsRoles" :key="item.id" :label="item.roleName" :value="item.id">
        </el-option>
      </el-select>
